...
if(program.list) {
	var t = new Table({head: ["Function Name", "Renditions"], colWidths: [40, 20]});
...
if(!program.noWrap) {
	// write the header
	buffer.push(fs.readFileSync(program.header, "utf8"));
}
...
order.forEach(function(f, i){
	buffer.push("\t\"" + f + "\": " + f + (i === order.length-1 ? "" : ",") + "\n");
});
buffer.push("};\n");

// write the footer
if(!program.noWrap) {
	buffer.push(fs.readFileSync(program.footer, "utf8"));
}
